...
public class JDBCExample_HSql_Mem extends AbstractVerticle {
	...
	public static void main(String[] args) {
		...
		Runner.runExample(JDBCExample_HSql_Mem.class);
	}
	...
	public void start() throws Exception {
		...
		try {
			...
			System.out.println("-----" + this.vertx.getOrCreateContext().config().getString("context-path"));
			
			configObj = this.vertx.getOrCreateContext().config();
			queries = this.vertx.getOrCreateContext().config()
					.getJsonObject("objectstoredao-queries")
					//.getJsonObject(vertx.getOrCreateContext().config().getString("database"))
					;
		} catch (Exception e) {
			e.printStackTrace();
		}
